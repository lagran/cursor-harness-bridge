...
- `deploy/harness-mobile.css` also turns Harness' desktop three-column grid
  ...
  transcript into a narrow strip, and the transcript header collapses its
  mode badge / "Session log" label to icons so the session title stays
  readable.
- `deploy/harness-mobile-nav.js` adds the matching behaviors: picking a
  session auto-closes the sheet, and tapping the scrim (or pressing Escape)
  closes it. Closing always goes through Harness' own "Collapse sidebar"
  button so React state stays authoritative.
...
Harness' `Read Only` / `Workspace Write` / `Full Access` selector belongs to
the Harness tool runtime, which this bridge replaces. Cursor SDK execution
policy is deployment-owned and defaults to:
...
```bash
# Safe default, plus another writable workspace root:
CURSOR_SANDBOX=true
CURSOR_AUTO_REVIEW=true
CURSOR_ADDITIONAL_DIRS=/path/to/another/project

# Deployment-wide Cursor Full Access:
CURSOR_SANDBOX=false
CURSOR_AUTO_REVIEW=false
```

`CURSOR_ADDITIONAL_DIRS` and `CURSOR_SETTING_SOURCES` are comma-separated.
Restart Harness after changing these process-level settings. The composer
permission badge does not override them.
